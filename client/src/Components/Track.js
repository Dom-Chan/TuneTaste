...
export default function Track({ item, id, type, albumImg }) {
  ...
  const { access_token, device, setPlaying } = useProps();
  ...
  const play = () => {
    ...
    if (type === "artist" || type === "album" || type === "track"){
      uri_holder = item.uri
    }else {uri_holder = item.track.uri}
  ...
  return (
    <div className={styles.track}>
      ...
      {(type === "artist" || type === "track") && <img src={item.album.images[0].url} alt=" " onClick={play}></img>}
      {type === "album" && <img src={albumImg} alt=" " onClick={play}></img>}
      {type === "playlist" && <img src={item.track.album.images[0].url} alt=" " onClick={play}></img>}

      {(type === "artist" || type === "album" || type === "track") && <div className={styles.track_names_wrapper}>
        <div className={styles.track_name}>{item.name}</div>
        <div className={styles.track_artist}>{artists}</div>
      </div>}

      {type === "playlist" && <div className={styles.track_names_wrapper}>
      ...
      {showModal && (
        <ReviewModal
          ...
          title={(type === "artist" || type === "album" || type === "track") ? item.name: item.track.name}
          type="Track"
          id={id}
        />
      )}
